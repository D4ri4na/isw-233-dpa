import { BLOG_ARTICLES, FavoritesStore } from '../../../entities/article/model/store.ts';
import { initIntersectionObserver, initMutationObserver } from '../../../shared/lib/observers.ts';
import type { ToggleFavDetail } from '../../../types.ts';

export function blogPage(): HTMLElement {
  ...
  const grid    = wrapper.querySelector<HTMLElement>('#blog-grid')!;
  const filters = wrapper.querySelector<HTMLElement>('#blog-filters')!;
  ...
  filters.addEventListener('click', (e: MouseEvent) => {
    const btn = (e.target as Element).closest<HTMLButtonElement>('.blog__filter-btn');
    if (!btn) return;
    filters
      .querySelectorAll<HTMLButtonElement>('.blog__filter-btn')
  ...
  wrapper.addEventListener('toggle-fav', (e: Event) => {
    const { id } = (e as CustomEvent<ToggleFavDetail>).detail;
