...
export function homePage(): HTMLElement {
  ...
  const grid = section.querySelector<HTMLElement>('#home-blog-grid')!;
  ...
  section.addEventListener('toggle-fav', (e: Event) => {
    const { id } = (e as CustomEvent<ToggleFavDetail>).detail;
    FavoritesStore.toggle(id);
    const card = section.querySelector<HTMLElement>(`blog-card[article-id="${id}"]`);
    if (card) card.setAttribute('favorited', FavoritesStore.has(id) ? 'true' : 'false');
  });
